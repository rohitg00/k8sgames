...
export class SettingsPanel {
  ...
  _buildHTML() {
    ...
    return `
      <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" id="ig-settings-backdrop"></div>
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[480px] max-h-[80vh] overflow-y-auto rounded-xl border border-white/10 bg-gray-900/95 backdrop-blur-xl shadow-2xl">
        <div class="flex items-center justify-between px-6 py-4 border-b border-white/5">
          ...
          <button id="ig-settings-close" class="p-1 text-white/30 hover:text-white/60 transition-colors">
        ...
        <div class="px-6 py-4 space-y-6">
          ...
          <div>
            ...
            <div class="flex items-center gap-3">
              <input type="range" id="ig-settings-camera-speed" min="0.5" max="2.0" step="0.1" value="${this.settings.cameraSpeed}" class="flex-1 h-1 bg-white/10 rounded-full appearance-none cursor-pointer accent-sky-400">
              <span id="ig-settings-camera-speed-val" class="text-white/60 text-xs font-mono w-8">${this.settings.cameraSpeed}x</span>
            </div>
          </div>
          ...
          <div class="pt-2 border-t border-white/5">
            ...
            <div class="flex gap-2">
              <button id="ig-settings-reset-progress" class="px-3 py-1.5 text-xs text-red-400 border border-red-500/20 rounded-lg hover:bg-red-500/10 transition-colors">Reset All Progress</button>
              <button id="ig-settings-export" class="px-3 py-1.5 text-xs text-white/50 border border-white/10 rounded-lg hover:bg-white/5 transition-colors">Export Save</button>
            </div>
          </div>
        </div>

        <div class="px-6 py-3 border-t border-white/5 flex justify-end">
          <button id="ig-settings-done" class="px-4 py-1.5 text-sm text-sky-400 bg-sky-400/10 rounded-lg hover:bg-sky-400/20 transition-colors font-medium">Done</button>
        </div>
      </div>
    `;
  }
  ...
  _bindEvents() {
    this.container.querySelector('#ig-settings-backdrop').addEventListener('click', () => this.hide());
    this.container.querySelector('#ig-settings-close').addEventListener('click', () => this.hide());
    this.container.querySelector('#ig-settings-done').addEventListener('click', () => this.hide());
    ...
    const slider = this.container.querySelector('#ig-settings-camera-speed');
    if (slider) {
      slider.addEventListener('input', () => {
        this.settings.cameraSpeed = parseFloat(slider.value);
        this.container.querySelector('#ig-settings-camera-speed-val').textContent = this.settings.cameraSpeed + 'x';
        this._saveSettings();
        this._applySettings();
      });
    }

    this.container.querySelector('#ig-settings-reset-progress').addEventListener('click', () => {
    ...
    this.container.querySelector('#ig-settings-export').addEventListener('click', () => {
      const data = {
